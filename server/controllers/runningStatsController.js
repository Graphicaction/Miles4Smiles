...
module.exports = {
  ...
  findById: function (req, res) {
    if (req.user) {
      db.User.find({ _id: req.user._id })
        ...
        .then((users) => {
          const runningStat = users[0].runningStats.filter(
  ...
  create: function (req, res) {
    db.RunningStat.create(req.body)
      .then((dbRunningStat) => {
        return db.User.findOneAndUpdate(
          { _id: req.user._id },
          { $push: { runningStats: dbRunningStat._id } },
          { new: true }
        );
      })
      .then((dbUser) => {
  ...
  update: function (req, res) {
    db.RunningStat.findOneAndUpdate({ _id: req.params.id }, req.body)
      .then((dbModel) => {
        console.log(dbModel);
        res.json(dbModel);
      })
  ...
  remove: function (req, res) {
    db.User.findOneAndUpdate(
      { _id: req.user._id },
      { $pull: { runningStats: new ObjectId(req.params.id) } },
      { new: true }
    ).then(() => {
      db.RunningStat.findOneAndDelete({ _id: req.params.id })
